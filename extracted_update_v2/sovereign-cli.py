...
from engines.execution.fyers_client import FyersExecutionEngine, signals_to_orders
...
def live_trade(
    dry_run:     bool  = typer.Option(True,   "--dry-run/--execute",  help="Dry run (default). Pass --execute to place real orders."),
    paper:       bool  = typer.Option(False,  "--paper",              help="Paper trade mode (fake orders, no real capital)."),
    capital:     float = typer.Option(1_000_000.0, "--capital",       help="Capital to deploy (post Kelly deduction)."),
    universe:    str   = typer.Option("STANDARD", "--universe",       help="Ticker universe to scan."),
    product:     str   = typer.Option("CNC",       "--product",       help="Fyers product type: CNC or INTRADAY."),
) -> None:
    """Run QuantOrchestrator and place live orders via Fyers."""

    started_at = int(time.time())
    args = {"dry_run": dry_run, "paper": paper, "capital": capital}
    guard = ModelGuard()
    if guard.is_fallback_active():
        console.print(f"[yellow]⚠️  Model guard active — ML fallback mode ({guard.status()['fallback_reason']})[/yellow]")
        console.print("[yellow]   Run with care: scores are rule-based, not ML-predicted.[/yellow]")

    try:
        from engines.quant_orchestrator import QuantOrchestrator
        from engines.portfolio.optimizer import PortfolioConstraints
        import asyncio

        # Step 1: Run full pipeline scan
        console.print(f"[cyan]Scanning {universe} universe...[/cyan]")
        pipeline_result = asyncio.run(_pipeline().run(
            positional_tickers=get_universe(universe),
            triggered_by="cli_live_trade",
        ))

        # Step 2: Run quant orchestrator for regime + Kelly + risk
        console.print("[cyan]Running quant orchestrator...[/cyan]")
        quant = QuantOrchestrator(
            total_capital=capital,
            constraints=PortfolioConstraints(max_position_pct=0.08, max_sector_pct=0.30),
        )
        buy_signals = [
            {"ticker": r.ticker, "action": r.action,
             "score": r.score or 0, "price": 0}
            for r in pipeline_result.results
            if r.action in ("BUY", "WATCH")
        ]
        # Fetch prices for signal records
        fetcher = _fetcher()
        for sig in buy_signals:
            try:
                data = fetcher.fetch(sig["ticker"])
                sig["price"] = data.price or 0
            except Exception:
                pass

        if not buy_signals:
            console.print("[yellow]No BUY/WATCH signals — no orders to place.[/yellow]")
            return

        returns_df = None  # quant orchestrator uses fallback if None
        quant_result = quant.run(buy_signals, returns_df)

        # Step 3: Convert weights to Fyers orders
        weights = {p.ticker: p.weight for p in quant_result.portfolio.positions}
        kelly_cash = 1.0 - sum(weights.values())
        effective_capital = capital * (1.0 - kelly_cash)
        ...
        orders = signals_to_orders(
            ...
            product=product,
            dry_run=True,  # always compute orders, dry_run controls placement
        )

        # Step 4: Display order table
        table = Table(title=f"{'DRY RUN — ' if dry_run else ''}Live Orders")
        table.add_column("Ticker"); table.add_column("Side")
        table.add_column("Qty", justify="right"); table.add_column("Est. Price", justify="right")
        table.add_column("Weight", justify="right"); table.add_column("Alloc ₹", justify="right")
        for order in orders:
            sig = next((s for s in buy_signals if s["ticker"] in order.ticker), None)
            price_str = f"₹{sig['price']:,.1f}" if sig and sig["price"] else "market"
            wt = weights.get(sig["ticker"] if sig else "", 0)
            alloc = f"₹{wt * effective_capital:,.0f}"
            table.add_row(order.ticker, order.side, str(order.quantity), price_str,
                          f"{wt:.1%}", alloc)
        console.print(table)
        console.print(
            f"\nRegime: [bold]{quant_result.regime.regime}[/bold] | "
            f"Positions: {len(orders)} | "
            f"Kelly cash: {kelly_cash:.1%} | "
            f"Deploying: \u20b9{effective_capital:,.0f}"
        )

        if dry_run:
            console.print("\n[yellow]DRY RUN — no orders placed. Use --execute to place real orders.[/yellow]")
            _record_cli_run("live_trade", args, "PASS", f"Dry run: {len(orders)} orders computed", started_at)
            return

        # Step 5: Place orders
        if not paper:
            confirm = typer.confirm(f"Place {len(orders)} LIVE orders for ₹{effective_capital:,.0f}?")
            if not confirm:
                console.print("[yellow]Aborted.[/yellow]")
                return

        engine  = FyersExecutionEngine(paper_trade=paper)
        results = engine.place_batch(orders)
        placed  = sum(1 for r in results if r.success)
        failed  = len(results) - placed

        res_table = Table(title="Execution Results")
        res_table.add_column("Ticker"); res_table.add_column("Status")
        res_table.add_column("Order ID"); res_table.add_column("Fill ₹", justify="right")
        for r in results:
            status_str = "[green]FILLED[/green]" if r.success else "[red]FAILED[/red]"
            res_table.add_row(r.ticker, status_str, r.order_id or "—",
                              f"₹{r.fill_price:,.1f}" if r.fill_price else "—")
        console.print(res_table)
        console.print(f"\nPlaced: {placed} | Failed: {failed}")
        summary = f"Placed {placed}/{len(orders)} orders"
        _record_cli_run("live_trade", args, "PASS" if failed == 0 else "WARN", summary, started_at)

    except Exception as exc:
        _handle_command_failure("live_trade", args, exc, started_at)


@app.command("metrics")
def metrics_server(
    port:      int  = typer.Option(9090, "--port",  help="Port for Prometheus metrics endpoint."),
    once:      bool = typer.Option(False, "--once", help="Print metrics once and exit (no server)."),
    update_db: bool = typer.Option(True, "--update-db/--no-update-db", help="Pull fresh values from DB before printing."),
) -> None:
    """Start Prometheus metrics HTTP server or print metrics once."""

    if update_db:
        sovereign_metrics.update_from_db()

    if once:
        console.print(sovereign_metrics.export())
        return

    t = sovereign_metrics.start_server(port=port)
    console.print(f"[green]Metrics server started on :[bold]{port}[/bold]/metrics[/green]")
    console.print("Add Prometheus scrape target: http://localhost:{port}/metrics")
...
@app.command("model-guard")
def model_guard_cmd(
    status:        bool = typer.Option(False, "--status",        help="Show current guard status."),
    force_fallback: bool = typer.Option(False, "--force-fallback", help="Force activate rule-based fallback."),
    clear:         bool = typer.Option(False, "--clear",         help="Clear fallback and return to ML mode."),
) -> None:
    """Inspect or control the ML model decay guard."""

    guard = ModelGuard()
    if force_fallback:
        guard.force_fallback("manual via CLI")
        console.print("[yellow]Fallback activated — rule-based scoring now active.[/yellow]")
    elif clear:
        guard.clear_fallback()
        console.print("[green]Fallback cleared — ML scoring restored.[/green]")

    s = guard.status()
    table = Table(title="Model Guard Status")
    table.add_column("Field"); table.add_column("Value")
    table.add_row("Fallback active", "[red]YES[/red]" if s["fallback_active"] else "[green]NO[/green]")
    table.add_row("Reason", s.get("fallback_reason") or "—")
    table.add_row("Outcome samples", str(s.get("outcome_samples", 0)))
    table.add_row("Baseline AUC", str(s.get("baseline_auc", "—")))
    table.add_row("Model ID", (s.get("current_model_id") or "—")[:16])
    console.print(table)


@app.command("cycle")
def cycle_detection() -> None:
    """Detect current NSE market cycle phase."""

    try:
        from engines.regime.regime_engine import RegimeEngine
        payload = RegimeEngine().classify().to_payload()
        market_data = {
            "breadth_slope_20d":   (payload.get("composite_score", 50) - 50) / 50,
            "cyclical_vs_defensive": (payload.get("signals", {}).get("sector_strength", 50) - 50) / 5,
            "gsec_10y":            6.85,
            "repo_rate":           6.5,
            "net_fii_20d_cr":      (payload.get("signals", {}).get("fii_flow", 50) - 50) * 200,
        ...
        table = Table(title=f"Market Cycle: {result.phase} (confidence {result.confidence:.0%})")
        table.add_column("Signal"); table.add_column("Vote")
        for sig, vote in result.signal_votes.items():
            table.add_row(sig, vote)
        console.print(table)

        console.print(f"\n[bold]Phase:[/bold]        {result.phase}")
        console.print(f"[bold]Description:[/bold]  {result.description}")
        console.print(f"[bold]Leaders:[/bold]      {', '.join(result.sector_leaders[:3])}")
        console.print(f"[bold]Laggards:[/bold]     {', '.join(result.sector_laggards[:3])}")
        console.print(f"[bold]Position bias:[/bold] {result.position_bias:.2f}×")
    except Exception as exc:
        _handle_command_failure("cycle", {}, exc, int(time.time()))
